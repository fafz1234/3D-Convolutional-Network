...
def ProcessCommandLine():
    ...
    return args.tc_data_dir, args.td_data_dir, args.scae_model, args.cae1_model, args.cae2_model, args.cae3_model, args.activation_cae, \
...
def main():
    data_dir_control, data_dir_disease, scae_model, cae1_model, cae2_model, cae3_model, activation_cae, activation_final, \
    ...
    for data in data_list:
        mri_control = nib.load(data)
        if control is None:
            control = mri_control.get_data()
            np.expand_dims(control, axis=0)
        else:
            control = np.stack( (control, mri_control.get_data()), axis=0 )

    data_list = os.listdir(data_dir_disease)
    for data in data_list:
        mri_disease = nib.load(data)
        if disease is None:
            disease = mri_disease.get_data()
            np.expand_dims(disease, axis=0)
        else:
            disease = np.stack( (disease, mri_disease.get_data()), axis=0 )
    ####################################

    depth, height, width = control[0].shape
    in_channels   = 1
    in_time       = depth
    in_width      = width
    in_height     = height
